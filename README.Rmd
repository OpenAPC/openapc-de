...
At the moment, the dataset releases information on `r nrow(my.apc)` articles, with total expenditure of `r format(sum(my.apc$EURO), scientific=FALSE)`€. Average fee is `r format(sum(my.apc$EURO)/nrow(my.apc), digits = 5)`€.
...
p <- ggplot(my.df, aes(publisher, x, fill= Uni)) + geom_bar(stat="identity")  + ylab("Fees paid (in EURO)") + coord_flip() + 
  scale_y_continuous(limits=c(0, 700000), labels=function(x) format(x, big.mark = ".", scientific = FALSE)) + 
