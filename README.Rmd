...
my_costs <- readr::read_csv("data/apc_additional_costs.csv")
...
my_cost_types <- my_costs %>%
  select("Colour charges","Cover","Other","Page charges","Permission","Publication charges","Reprint","Submission fee")
  
costs_summary <- tibble(
  type = c("Colour charges","Cover","Other","Page charges","Permission","Publication charges","Reprint","Submission fee"),
...
p <- my_apc %>% 
  inner_join(my_costs) %>% 
  select(publisher,euro,"Colour charges","Cover","Other","Page charges","Permission","Publication charges","Reprint","Submission fee") %>%
  filter(publisher %in% c("IOP Publishing", "Springer Nature", "MDPI AG", "Wiley-Blackwell", "AIP Publishing", "Elsevier BV", "Frontiers Media SA", "Public Library of Science (PLoS)", "Oxford University Press (OUP)", "American Chemical Society (ACS)")) %>%
  pivot_longer(., cols=c(euro,"Colour charges","Cover","Other","Page charges","Permission","Publication charges","Reprint","Submission fee"), values_drop_na = TRUE) %>%
  ggplot(aes(publisher, value, fill = name)) +
  geom_bar(stat="identity") +
  xlab("Publisher") +
  ylab("Euro") +
  coord_flip()
  ggsave(p, path = "figure/", filename = "juelich_additional_costs.png", width=9, height=4.5, units="in", device = "png")
```
