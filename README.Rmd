...
my_cost_types <- my_costs %>%
  select(colour_charges, cover, other, page_charges, reprint)
  
costs_summary <- tibble(
  type = c("Colour charges", "Cover", "Other", "Page charges", "Reprint"),
  total = vector("double", 5),
  count = vector("integer", 5),
  median = vector("double", 5)
)
...
p <- my_costs %>% 
  left_join(my_apc) %>% 
  select(publisher, euro, colour_charges, cover, other, page_charges, reprint) %>%
  pivot_longer(., cols=c(euro, colour_charges, cover, other, page_charges, reprint), values_drop_na = TRUE) %>%
  ggplot(aes(publisher, value, fill = name)) +
  geom_bar(stat="identity") +
  coord_flip()
  ggsave(p, path = "figure/", filename = "additional_avg_costs.png", width=9, height=4.5, units="in", device = "png")
```
