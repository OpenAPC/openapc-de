...
#only unis
my.apc <- my.apc[!my.apc$Institution %in% c("MPG", "FZJ - ZB"),]
...
p <- tt + ylab("Fees paid (in EURO)") + coord_flip() + scale_y_continuous(limits=c(0, 350000), labels=function(x) format(x, big.mark = ".", scientific = FALSE)) + 
  scale_fill_brewer(type="qual", palette = 1) + theme_bw()

p <- p + theme(legend.position="top", legend.text = element_text(size = 4, colour = "black"))
...
q <- q +  ylab("Fees paid (in EURO)") + theme(legend.position="top") + theme_bw()
